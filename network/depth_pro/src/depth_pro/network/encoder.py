...
class DepthProEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.out_size = int(
            patch_encoder.patch_embed.img_size[0] // patch_encoder.patch_embed.patch_size[0] // 4
        )
    ...
    def _hook0(self, model, input, output):
        ...
        # self.backbone_highres_hook0[input[0].device] = output

    def _hook1(self, model, input, output):
        self.backbone_highres_hook1 = output
        # self.backbone_highres_hook1[input[0].device] = output
    ...
    def _create_pyramid(
        ...
        # Original resolution: 384 by default.
        x0 = x

        # resolution: 192 by default.
        x1 = F.interpolate(
            x, size=None, scale_factor=0.5, mode="bilinear", align_corners=False
        )

        # Low resolution: 96 by default, corresponding to the backbone resolution.
        x2 = F.interpolate(
    ...
    def forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        ...
        # resolution.
        # 5x5 @ 96x96 at the highest resolution (384x384).
        x0_patches = self.split(x0, overlap_ratio=0.25)
        # 3x3 @ 96x96 at the middle resolution (192x192).
        x1_patches = self.split(x1, overlap_ratio=0.5)
        # 1x1 # 96x96 at the lowest resolution (96x96).
        x2_patches = x2
        ...
        # 24x24 feature maps by merging 5x5 @ 12x12 patches with overlaps.
        x0_features = self.merge(x0_encodings, batch_size=batch_size, padding=1)
        x0_features = nn.functional.interpolate(
                x0_features,
                size=(24, 24),
                mode='bilinear',
                align_corners=False,
        ...
        # 12x12 feature maps by merging 3x3 @ 12x12 patches with overlaps.
        x1_features = self.merge(x1_encodings, batch_size=batch_size, padding=2)
        x1_features = nn.functional.interpolate(
                x1_features,
                size=(12, 12),
                mode='bilinear',
                align_corners=False,
        ...
        x_latent0_features = nn.functional.interpolate(
                x_latent0_features,
                size=(192, 192),
                mode='bilinear',
                align_corners=False,
        ...
        x_latent1_features = nn.functional.interpolate(
                x_latent1_features,
                size=(96, 96),
                mode='bilinear',
                align_corners=False,
