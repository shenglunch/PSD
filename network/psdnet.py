...
from .submodules import Conv2D, Deconv2D, BasicBlock
from .submodules import BasicBlockReZero as BBlockReZero

# Maybe you need to comment out some models depending on your conda environment.
...
# from .prompt_da.promptda.promptda import PromptDA as promptda
# from .metric3d.mono.metric3d import get_model as metric3d
# from .unidepth.unidepth.models import UniDepthV2
# from .unidepth.unidepth.utils.camera import Pinhole, MEI, Spherical
...
def get_promptda(pretrained):
    model = promptda.from_pretrained(pretrained)
    return model

def get_metric3d(backbone, pretrained):
    model = metric3d(backbone, pretrained)
    return model

def get_unidepth(backbone, pretrained):
    with open(os.path.join(backbone)) as f:
        config = json.load(f)
    model = UniDepthV2(config)
    info = model.load_state_dict(torch.load(pretrained), strict=False)
...
class ResidualBranchRes(nn.Module):
    def __init__(self, cfg, resnet):
        ...
        branch_dim = cfg.RegressBranch.branch_dim
        self.backbone = cfg.MDEBranch.backbone

        if self.backbone == 'promptda' or self.backbone == 'metricv2' or self.backbone == 'unidepth':
            self.mconvs = nn.ModuleList()
            for l in layer_dim:
                self.mconvs.append(nn.Sequential(
                    Conv2D(l, branch_dim, kernel_size=1, stride=1, padding=0, activation=None, norm=None),))
            self.mconv = nn.Sequential(
                Conv2D(branch_dim*len(layer_dim), branch_dim, kernel_size=3, stride=1, padding=1, activation='lrelu', norm='rezero'),
                Conv2D(branch_dim, branch_dim, kernel_size=3, stride=1, padding=1, activation='lrelu', norm='rezero'),)
        else:
            self.mconv1 = nn.Sequential(
                Conv2D(layer_dim[0], branch_dim, kernel_size=1, stride=1, padding=0, activation=None, norm=None),)
            self.mconv2 = nn.Sequential(
                Conv2D(layer_dim[1], branch_dim, kernel_size=1, stride=1, padding=0, activation=None, norm=None),)
            self.mconv3 = nn.Sequential(
                Conv2D(layer_dim[2], branch_dim, kernel_size=1, stride=1, padding=0, activation=None, norm=None),)
            self.mconv4 = nn.Sequential(
                Conv2D(layer_dim[3], branch_dim, kernel_size=1, stride=1, padding=0, activation=None, norm=None),)
            self.mconv = nn.Sequential(
                Conv2D(branch_dim*4, branch_dim, kernel_size=3, stride=1, padding=1, activation='lrelu', norm='rezero'),
                Conv2D(branch_dim, branch_dim, kernel_size=3, stride=1, padding=1, activation='lrelu', norm='rezero'),)
        ...
        self.all_fuse = GateFusion(resnet_dim+branch_dim, resnet_dim)
    ...
    def forward(self, sparse, depth_scale, residual, mde_feats):           
        ...
        if self.backbone == 'promptda' or self.backbone == 'metricv2' or self.backbone == 'unidepth':
            ms = []
            for idx, (conv, f) in enumerate(zip(self.mconvs, mde_feats)):
                ms.append(F.interpolate(conv(f), size=x1.shape[2:], mode='bilinear', align_corners=False))
            m = self.mconv(torch.cat(ms, dim=1))
        else:
...
class ImgBranchRes(nn.Module):
    def __init__(self, cfg, resnet):
        ...
        branch_dim = cfg.CSLBranch.ImgBranch.branch_dim
        self.backbone = cfg.MDEBranch.backbone
...
class MDEBranch(nn.Module):
    ...
    def forward(self, x):
        ...
        elif self.cfg.MDEBranch.backbone == 'promptda':
            prompt_depth = x['sparse'] # x['ip'] # 192x256, ARKit LiDAR depth in meters
            depth_non_scale, path_feats = self.net.predict(x['rgb_mde'], prompt_depth)

            depth_non_scale = depth_non_scale.float() + 1e-5
            depth_non_scale = 1/depth_non_scale
            depth_non_scale = torch.clamp(depth_non_scale, min=1e-3)
        elif self.cfg.MDEBranch.backbone == 'metricv2':
            x['input'] = x['rgb_mde']
            pred_depth, confidence, output_dict = self.net.inference(x)
            path_feats = output_dict['feats']

            pad_info = x['pad_info']
            normalize_scale = x['normalize_scale']
            scale_info = x['scale_info']
            size = x['input'].shape[2:]

            pred_depth = pred_depth[:, :, pad_info[0][0] : pred_depth.shape[2] - pad_info[1][0], 
                                        pad_info[2][0] : pred_depth.shape[3] - pad_info[3][0]]
            pred_depth = F.interpolate(pred_depth, size, mode='bilinear', align_corners=False) # to original size
            normalize_scale= normalize_scale.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            scale_info = scale_info.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
            
            depth_non_scale = pred_depth * normalize_scale / scale_info
            
            depth_non_scale = depth_non_scale.float() + 1e-5
            depth_non_scale = 1/pred_depth
            depth_non_scale = torch.clamp(depth_non_scale, min=1e-3)
        elif self.cfg.MDEBranch.backbone == 'unidepth':
            # bug
            # if x['cam'][0] == 'pinhole':
            #     camera = Pinhole(K=x['K'])
            # elif x['cam'][0] == 'MEI':
            #     camera = MEI(params=x['M2'])
            # elif x['cam'][0] == '360':
            #     camera = Spherical(x['S'])

            camera = Pinhole(K=x['K'])

            out = self.net.infer(x['rgb_mde'], camera)
            depth_non_scale = out['depth']

            depth_non_scale = depth_non_scale.float() + 1e-5
            depth_non_scale = 1/depth_non_scale
            depth_non_scale = torch.clamp(depth_non_scale, min=1e-3)
            
            path_feats = [out['psd_feat'][0].float(), out['psd_feat'][1].float(), out['psd_feat'][2].float()]
...
class DCNet(nn.Module):
    def __init__(self, cfg):
        ...
        #     promptda = get_promptda(cfg.MDEBranch.promptda.pretrained)
        #     self.mde = MDEBranch(cfg, promptda)
        #     del promptda
        # elif cfg.MDEBranch.backbone == 'metricv2':
        #     metricv2 = get_metric3d(cfg.MDEBranch.metricv2.backbone, cfg.MDEBranch.metricv2.pretrained)
        #     self.mde = MDEBranch(cfg, metricv2)
        #     del metricv2
        # elif cfg.MDEBranch.backbone == 'unidepth':
            unidepth = get_unidepth(cfg.MDEBranch.unidepth.backbone, cfg.MDEBranch.unidepth.pretrained)
            self.mde = MDEBranch(cfg, unidepth)
            del unidepth
        
        if cfg.RegressBranch.backbone == 'resnet18':
            resnet = get_resnet18(cfg.RegressBranch.resnet.pretrained)
            self.rb = ResidualBranchRes(cfg, resnet)
            del resnet
        
        if cfg.CSLBranch.backbone == 'resnet18':
    ...
    # depth_inverse: depth anything(v2)/midas的输出 不用其他处理
    # target: 真实深度图/稀疏深度图
