...
class DepthHead(nn.Module):
    ...
    def depth_proj(self, out_features):
        ...
        for i, (layer, features) in enumerate(zip(self.depth_mlp, out_features)):
            ...
            psd_feat.append(out_depth_features)
    ...
    def decode(self, out_features):
        logdepth, psd_feat = self.depth_proj(out_features)
        confidence = self.confidence_proj(out_features)
        return logdepth, confidence, psd_feat
    ...
    def forward(
        ...
        logdepth, logconf, psd_feat = self.decode(features)

        return logdepth, logconf, proj_latents_16, psd_feat
...
class Decoder(nn.Module):
    ...
    def forward(
        ...
        logdepth, logconfidence, depth_features, psd_feat = self.depth_layer(
            features=features,
            rays_hr=rays,
            pos_embed=pos_embed,
