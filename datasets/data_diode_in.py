...
class DIODEiDataset(data.Dataset):
    def __init__(self, cfg, mode='train'):
        super(DIODEiDataset, self).__init__()
        self.cfg = cfg
        self.mode = mode
        self.rate = cfg.DIODEi.rate
        self.max = cfg.DIODEi.max
        
        self.valrgb = cfg.DIODEi.valrgb
        self.valgt = cfg.DIODEi.valgt
        self.valraw = cfg.DIODEi.valraw
        
        if mode== 'train':
            pass
        elif mode== 'val':
            self.files = self.load_path(cfg.DIODEi.vallist)
        ...
        if cfg.MDEBranch.backbone == 'midas':
            ...
            self.data_basic = cfg.DIODEi.data_basic
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            rgb_file = os.path.join(self.valrgb, self.files[index])
            target_file = os.path.join(self.valgt, self.files[index])
            sparse_file = os.path.join(self.valraw, self.files[index])

            rgb = self.get_rgb(rgb_file)
            target = self.get_depth(target_file)
            sparse = self.get_depth(sparse_file)
            K = np.reshape(K_d, [3, 3])
