...
K_lm = [
    2262.52,                   0, 1096.98, \
    0.00   ,  2265.3017905988554, 513.137, \
    0.00   ,  0.000             , 1.000]
b_lm = 0.209313
K_f = [
    2268.36,                   0, 1048.64, \
    0.00   ,  2225.5405988775956, 519.277, \
    0.00   ,  0.000             , 1.000]
b_f = 0.222126
...
def val_transform(rgb, target, K, cfg):
    ...
    h_c = cfg.Cityscape.center_height
    w_c = cfg.Cityscape.center_width
...
class CityscapeDataset(data.Dataset):
    def __init__(self, cfg, mode='train'):
        ...
        self.cfg = cfg
        self.mode = mode
        self.rate = cfg.Cityscape.rate
        self.max = cfg.Cityscape.max
        self.valrgb = cfg.Cityscape.valrgb
        self.valgt = cfg.Cityscape.valgt
        self.valraw = cfg.Cityscape.valraw
        self.valk = cfg.Cityscape.valk

        self.K_f = np.reshape(K_f, [3, 3])
        self.K_lm = np.reshape(K_lm, [3, 3])
        self.b_lm = b_lm
        self.b_f = b_f
        ...
        if mode== 'train':
            ...
            self.rgb_files, self.sparse_files, self.target_files, self.intrinsic_files = self.load_path(cfg.Cityscape.vallist)
        ...
        if cfg.MDEBranch.backbone == 'midas':
            ...
            self.data_basic = cfg.Cityscape.data_basic
        ...
        # self.trans_d2tensor = transforms.Compose([
        #         transforms.ResizeV2((cfg.Cityscape.resize_height, cfg.Cityscape.resize_width), 0),
        #         transforms.ToTensor(),])
