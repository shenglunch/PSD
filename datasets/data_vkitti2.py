...
class VKITTI2Dataset(data.Dataset):
    def __init__(self, cfg, mode='train'):
        super(VKITTI2Dataset, self).__init__()
        self.cfg = cfg
        self.mode = mode
        self.max = cfg.VKITTI2.max

        self.valrgb = cfg.VKITTI2.valrgb
        self.valgt = cfg.VKITTI2.valgt
        self.valraw = cfg.VKITTI2.valraw

        self.K = np.reshape([725.0087, 0.000000e+00, 620.5, \
                0.000000e+00, 725.0087, 187, \
                0.000000e+00, 0.000000e+00, 1.000000e+00], [3, 3])
        
        if mode== 'train':
            pass 
        elif mode== 'val':
            self.rgb_files, self.sparse_files, self.target_files = self.load_path(cfg.VKITTI2.vallist)
        ...
        if cfg.MDEBranch.backbone == 'midas':
            ...
            self.data_basic = cfg.VKITTI2.data_basic
