...
class SUNRGBDDataset(data.Dataset):
    def __init__(self, cfg, mode='train'):
        ...
        self.cfg = cfg
        self.mode = mode
        self.max = cfg.SUNRGBD.max
        
        self.valrgb = cfg.SUNRGBD.valrgb
        self.valgt = cfg.SUNRGBD.valgt
        self.valraw = cfg.SUNRGBD.valraw
        self.valk = cfg.SUNRGBD.valk
        
        if mode== 'train':
            pass 
        elif mode== 'val':
            self.rgb_files, self.sparse_files, self.target_files, self.intrinsic_files = self.load_path(cfg.SUNRGBD.vallist)
        ...
        if cfg.MDEBranch.backbone == 'midas':
            ...
            self.data_basic = cfg.SUNRGBD.data_basic
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            inputs['target'] = torch.where(torch.isnan(inputs['target']), 
                                torch.full_like(inputs['target'], 0), inputs['target'])
